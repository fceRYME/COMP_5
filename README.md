...
- Дополнительное задание выполнено с помощью графической библиотеки `PyQt6`.
- `QDialog` — отдельное окно для отображения графического AST.
- `QGraphicsScene` — сцена, на которой размещаются узлы и ребра дерева.
- `QGraphicsView` — область просмотра сцены с поддержкой прокрутки и перетаскивания.
- `QPainter` — включение сглаживания линий и текста.
- `QPen`, `QBrush`, `QColor`, `QFont`, `QRectF` — оформление прямоугольных блоков, линий, цветов и текста.
